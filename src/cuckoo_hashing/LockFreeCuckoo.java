...
public class LockFreeCuckoo {
	...
	static AtomicInteger [][]hashtable;

	//Array to store possible positions for a key
	static AtomicInteger []pos;
	...
	static void initTable() {
		for (int j = 0; j < MAXN; j++)
			for (int i = 0; i < ver; i++)
				hashtable[i][j] = new AtomicInteger(Integer.MIN_VALUE);
		for(int i = 0; i < ver; i++)
			pos[i] = new AtomicInteger();
	}
	
	public LockFreeCuckoo() {
		hashtable = new AtomicInteger[ver][MAXN];
		pos = new AtomicInteger[ver];
		initTable();
	}
	...
	void place(int key, int tableID, int cnt, int n) {
		 ...
		 * If YES, return. */
		for (int i = 0; i < ver; i++) {
			//System.out.println("pos["+ i +"]: " + hash(i + 1, key));
			pos[i].set(hash(i + 1, key));
			//System.out.println("hashtable["+ i +"]: " + hashtable[i][pos[i].get()].get());
			while(true) {
				int cur = hashtable[i][pos[i].get()].get();
				if (hashtable[i][pos[i].get()].compareAndSet(cur, key)) {
					return;
				} else {
					Thread.yield();
				}					
		...
		while(true) {
			if(hashtable[tableID][pos[tableID].get()].compareAndSet(Integer.MIN_VALUE, key)) {
				return;
			} else {
				int dis = hashtable[tableID][pos[tableID].get()].get();
				if(hashtable[tableID][pos[tableID].get()].compareAndSet(dis, key)) {
...
//			hashtable[tableID][pos[tableID]] = key;
	...
	void printTable() {
		...
		for (int i = 0; i < ver; i++, System.out.printf("\n"))
			for (int j = 0; j < MAXN; j++)
				if(hashtable[i][j].get() == Integer.MIN_VALUE)
					System.out.printf("- ");
				else
					System.out.printf("%d ", hashtable[i][j].get());
